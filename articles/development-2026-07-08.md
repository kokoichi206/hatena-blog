...
Claude Code は Bash ツールを実行する際、毎回 `.zshrc` を読み直しません。あらかじめシェルの状態をダンプしておいたファイルを読み込んで起動します。
...
ただ `ghq` 配下には自分のリポジトリと業務リポジトリしか置いていません。毎回聞かれるのが純粋にノイズでした。
...
zsh は対話シェルだと、デフォルトで `#` をコメント扱いしません。ただの引数として渡します。
...
- 起動ファイルは役割で分ける。`.zshenv` は環境変数だけ、`.zprofile` はログイン時に一度でいい初期化、`.zshrc` は対話シェル用
- 設定ファイルが伸びたら責務で分割する。総行数は変わらなくても、開くファイルが確定するだけで探す手間が消える
- 1 回あたりのコストが小さくても、回数が多いものは潰す価値がある。zoxide の警告や `mise trust` がそれにあたる

今回は「毎回ちょっとだけ手を動かしていたこと」を潰す変更が多めでした。個々は数秒の話ですが、`cd` のたび・リポジトリを移るたびに発生するので、消すと体感が変わります。
