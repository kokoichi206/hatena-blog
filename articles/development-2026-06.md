...
- PC: macOS / Apple Silicon (`nixpkgs.hostPlatform = "aarch64-darwin"`)
- Terminal: WezTerm
- Shell: zsh
...
`.config/wezterm/finder.lua` を追加し、開いている全 pane の cwd と foreground process、さらに zoxide の directory を同じ selector に並べるようにした。keybind は `LEADER + f`。
...
ターミナル上の `@` を含む文字列が mail address と解釈され、意図せずメーラーが開くことが多々あった。
